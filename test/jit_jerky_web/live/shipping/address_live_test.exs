defmodule JITJerkyWeb.Shipping.AddressLiveTest do
  ...
  @create_attrs %{address_line1: "some address_line1", address_line2: "some address_line2", city: "some city", country: "some country", delivery_instructions: %{}, name: "some name", postal_code: "some postal_code", province: "some province", telephone: "some telephone"}
  ...
  defp fixture(:address) do
    {:ok, address} = Shipping.create_address(@create_attrs)
    address
  end
  ...
  describe "Index" do
    ...
    test "lists all addresses", %{conn: conn, address: address} do
      {:ok, _index_live, html} = live(conn, Routes.shipping_address_index_path(conn, :index))
    ...
    test "saves new address", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, Routes.shipping_address_index_path(conn, :index))

      assert index_live |> element("a", "New Address") |> render_click() =~
               "New Address"

      assert_patch(index_live, Routes.shipping_address_index_path(conn, :new))
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, Routes.shipping_address_index_path(conn, :index))
    ...
    test "updates address in listing", %{conn: conn, address: address} do
      ...
      assert_patch(index_live, Routes.shipping_address_index_path(conn, :edit, address))
  ...
  describe "Show" do
    ...
    test "displays address", %{conn: conn, address: address} do
      {:ok, _show_live, html} = live(conn, Routes.shipping_address_show_path(conn, :show, address))
    ...
    test "updates address within modal", %{conn: conn, address: address} do
      {:ok, show_live, _html} = live(conn, Routes.shipping_address_show_path(conn, :show, address))

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Address"

      assert_patch(show_live, Routes.shipping_address_show_path(conn, :edit, address))
      ...
      {:ok, _, html} =
        ...
        |> form("#address-form", address: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, Routes.shipping_address_show_path(conn, :show, address))
